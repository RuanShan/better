  ...
  class BrokerMonthlyFactory
    def self.create(type, user_months, dates=[])
      ...
      monthly_results = grouped_user_months.map{|date, months|
        type == "profit" ? BrokerMonthlyProfit.new( date, months) : BrokerMonthlyBalance.new( date, months)
      }
      ...
      if type == "profit"
        ...
        unless results.empty?
          ...
          dates.each do |date|
            broker_balance  = results.select{|result| result.effective_on == date}.first || BrokerMonthlyBalance.new( date )
