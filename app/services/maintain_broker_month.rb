...
class MaintainBrokerMonth
  ...
  def run
    ...
    date = specified_date
    Broker.has_one :specified_date, ->{ where( effective_on: date )}, class_name: "BrokerDay"
    Broker.has_one :month_of_specifed_date, ->{ where( effective_on: first_day_of_month )}, class_name: "BrokerMonth"
